...
class ElfFile(StructBase):
    ...
    def __getnewargs__(self):
        return (self.name, self.fileIdent)
    ...
    def unpack_from(self, block, offset=0):
        ...
        # TODO
        # save mapping program headers -> sections
        #for i in xrange(len(self.sectionHeaders)):
            #print('sec %d [%x-%x]' % (i, self.sectionHeaders[i].offset, self.sectionHeaders[i].offset + self.sectionHeaders[i].section_size))
            #for j in xrange(len(self.programHeaders)):
                #if self.sectionHeaders[i].offset >= self.programHeaders[j].offset and self.sectionHeaders[i].offset + self.sectionHeaders[i].section_size <= self.programHeaders[j].offset + self.programHeaders[j].filesz:
                    #print('seg %d [%x-%x]' % (j, self.programHeaders[j].offset, self.programHeaders[j].offset + self.programHeaders[j].filesz))
                    #self.mapping[i] = j
                    #break
    ...
    def pack(self):
        ...
        x = bytearray(self.calc_size())
        self.pack_into(x)
        return x

    def calc_size(self):
        self._regen_section_name_table()
        total, scoff, shoff, pcoff, phoff = self._offsets(0)
        return total
    ...
    def _pack_sections(self, block, offset=0):
        ...
        """
        p = offset
        for section in sorted(self.sectionHeaders, key=operator.attrgetter('offset')):
            #print('_pack_sections: %s %x' % (section.name, section.offset))
            section.offset = p
            section.section_size = len(section.content)
            if section.type != SHT.byname['SHT_NOBITS'].code:
    ...
    def _pack_section_headers(self, block, offset):
        ...
        """
        shiter = list(zip(list(range(0, len(self.sectionHeaders))), self.sectionHeaders))
        for i, sh in shiter:
            sh.pack_into(block, offset + (i * self.fileHeader.shentsize))

    def _pack_program_headers(self, block, offset):
        """
        Pack the program header table.
        """
        shiter = list(zip(list(range(0, len(self.programHeaders))), self.programHeaders))
        for i, sh in shiter:
            sh.pack_into(block, offset + (i * self.fileHeader.phentsize))
    ...
    def __eq__(self, other):
        ...
        for this, that in zip(self.sectionHeaders, other.sectionHeaders):
            if this != that:
                ...
                print('differs from', file=sys.stderr)
                return False
